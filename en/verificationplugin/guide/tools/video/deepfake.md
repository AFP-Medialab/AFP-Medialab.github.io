...
**Deepfake video detection** is an experimental forensic environment engineered by ITI CERTH that analyses videos to identify AI-manipulated faces. Deepfakes use deep learning models (typically GANs or diffusion-based methods) to synthesize or swap faces in videos, often producing results that are difficult to detect with the naked eye.

This tool monitors video sequences via an ensemble of convolutional neural network (CNN) models, trained to recognise artefacts introduced by the synthesis process such as inconsistencies in facial blending, unnatural blinking patterns, and frequency-domain anomalies.
...
- This tool works only on facial modification by AI, it does not work on entirely AI generated video
